...
simulate.stochastic <- function( params, params_adaptive_tau) {
  ...
  # ── Build transitions ──────────────────────────────────────────────
  #transitions <- build_transitions_age(n, influx = params$influx)
  
  # ── Update params_adaptive_tau with current params and transitions ─
  #params_adaptive_tau$params      <- params
  #params_adaptive_tau$transitions <- transitions
  # params_adaptive_tau$relratechange <- rep(1, length(params_adaptive_tau$init))
